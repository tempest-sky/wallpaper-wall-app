            ...
            children: <Widget>[
              RefreshIndicator(
                ...
                child: CustomScrollView(
                  ...
                  slivers: <Widget>[
                    SliverToBoxAdapter(child: SizedBox(height: MediaQuery.paddingOf(context).top + 126)),
                    if (state.error != null)
                      SliverToBoxAdapter(
                        child: Padding(
                    ...
                    WallpaperGrid(
                      ...
                      onPlay: (wallpaper) => _openSlideshow(context, wallpaper),
                      onSave: (wallpaper) => _save(context, wallpaper),
                      onLoadMore: () => state.fetch(),
                    ),
                  ],
                ),
              ),
              AnimatedPositioned(
                ...
                child: _FloatingHeader(
                  ...
                  onFetch: () => state.fetch(reset: true),
                  onTop: _scrollToTop,
                ),
              ),
              if (state.selected != null)
                Positioned(
                  ...
                  child: _SelectedSheet(
                    ...
                    saving: state.saving,
                    onPlay: () => _openSlideshow(context, state.selected!),
                  ...
                  children: <Widget>[
                    DecoratedBox(
                      ...
                      child: const Padding(
                        ...
                        child: Icon(Icons.wallpaper_rounded, color: Colors.white),
                      ),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: <Widget>[
                          Text('Wallpaper Wall', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                          Text('$count 张 · ${selected.sourceLabel}', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant)),
                        ],
                      ),
                    ),
                    IconButton.filledTonal(onPressed: onTop, icon: const Icon(Icons.vertical_align_top_rounded)),
  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        ...
        boxShadow: <BoxShadow>[BoxShadow(color: Colors.black.withOpacity(0.28), blurRadius: 28, offset: const Offset(0, 14))],
      ),
      child: Padding(
        ...
        child: Row(
          children: <Widget>[
            ClipRRect(
              ...
              child: Image.network(wallpaper.url, width: 64, height: 86, fit: BoxFit.cover),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Text(wallpaper.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800)),
                  const SizedBox(height: 4),
                  Text(wallpaper.origin, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: scheme.onSurfaceVariant)),
                ],
              ),
            ),
            IconButton.filledTonal(onPressed: onPlay, icon: const Icon(Icons.play_arrow_rounded)),
            IconButton.filled(
              onPressed: saving ? null : onSave,
              icon: saving ? const SizedBox.square(dimension: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.download_rounded),
            ),
          ],
        ),
      ),
